...
Reset the manager-side target if one is active, then set the logical current vertex.
...
Foxy note: use `level: [1]`. If you send `level: 1`, Foxy can publish it as `level=b'\x00'`, which is treated as OK.
...
When the autonomy stack is started at vertex 1, keep the default. If the stack is restarted at another vertex, set both:
...
Current behavior publishes the first waypoint in the loaded leg continuously. For the current setup, keep one final target per leg such as `1_to_2.yaml` and `2_to_3.yaml`.
